...
export default function Home() {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post("http://127.0.0.1:8000/upload-dataset", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      setUploadStatus("✅ Uploaded! Ready to Train.");
    } catch (error) {
      console.error(error);
  ...
  const startTrainingProcess = async () => {
    ...
    setLoading(true);
    setLogs([]); 
    setStatus("⏳ Training Model (may take time)...");
    ...
    try {
      ...
      const accuracyInt = Math.floor(accuracy * 10000); 
    ...
    <div className="min-h-screen bg-gray-50 p-10 font-sans text-gray-800">
      <div className="flex justify-between items-center mb-8">
        ...
        {account ? (
          ...
          <button onClick={connectWallet} className="bg-orange-500 text-white px-6 py-2 rounded shadow hover:bg-orange-600">Connect Wallet</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {/* LEFT PANEL */}
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
          <h2 className="text-xl font-bold mb-4 border-b pb-2">1. Local Training Data</h2>
          
          {/* UPLOAD SECTION */}
          <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
            <h3 className="font-semibold text-blue-800 mb-2">Upload Custom Dataset (Zip)</h3>
            <input 
              type="file" 
              accept=".zip"
              onChange={handleFileChange}
              className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200 mb-2"
            />
            <div className="flex justify-between items-center">
              <button 
                onClick={handleUpload}
                disabled={!selectedFile}
                className={`text-xs px-3 py-1 rounded text-white ${!selectedFile ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}
              >
                Upload Dataset
              </button>
              <span className="text-xs font-mono text-gray-600">{uploadStatus}</span>
            </div>
            <p className="text-[10px] text-gray-400 mt-2">
              *Zip must contain folders for classes (e.g. 'Normal', 'Pneumonia').
            </p>
          </div>

          <div className="mb-4">
            <label className="block text-sm font-medium mb-1">Max Samples</label>
            <input 
              type="number" 
              value={sampleSize} 
              onChange={(e) => setSampleSize(Number(e.target.value))}
          ...
          <button
            ...
            disabled={loading || !account}
            className={`w-full py-3 rounded-lg font-bold text-white transition ${
              loading || !account ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"
            }`}
          >
            {loading ? "Training & Uploading..." : "Start Training Round"}
          </button>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
          ...
          <div className="overflow-auto h-[500px]">
             {/* Same as before... */}
             {updates.length === 0 ? (
              <div className="text-center text-gray-500 mt-10">No updates submitted.</div>
            ) : (
              <table className="w-full text-left">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="p-3 text-sm text-gray-500">Hospital</th>
                    <th className="p-3 text-sm text-gray-500">Acc</th>
                    <th className="p-3 text-sm text-gray-500">Hash</th>
                  </tr>
                </thead>
                <tbody>
                  {updates.map((u, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50">
                      <td className="p-3 font-mono text-xs text-blue-600">{u[0].slice(0, 6)}...</td>
                      <td className="p-3 font-bold text-green-600">{(Number(u[2]) / 100).toFixed(2)}%</td>
                      <td className="p-3"><span className="text-xs text-purple-600">{u[1].slice(0, 8)}...</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
